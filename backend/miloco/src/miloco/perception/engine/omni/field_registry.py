...
- ≤100 字；不用规则措辞、不因规则夸大
- 涉及人物用 identities 判出的姓名；unknown→"陌生人 / 某人"，不写"一人"这类泛称
...
- speaker：画面里看到某人正在说话（嘴动/说话姿态）、且其已在 identities 识别出 → 填其姓名；否则填"未知"。无声纹，不凭声音猜、不从 <gallery> 猜
...
- 仅人声。needs_response：仅当本句是对智能助手说的【信息查询】（问时间/天气/日程）、【设备控制】（开关/调节某设备）或【任务请求】（要助手去做/记某件事）才=true；家人间对话、自言自语、情绪感叹、纯陈述/闲聊、以及疑问句但显然在问家人而非助手的，一律=false
- speaker：本轮无画面、无身份信息，无法判断是谁在说话 → speaker 一律填"未知"（除非本人在话里自报姓名）。不凭声音猜、不臆测姓名
...
- reason 先写证据、再定 hit：hit=true 必须 reason 给出"规则每个要素都满足"的本轮证据——规则点名的人以 identities 为准，活动 / 状态只据本轮画面判断（听到的话 / 声音不作规则命中依据，音频不稳；见总原则）且不得与 caption 相矛盾；证据不全、靠推测、或与 caption / identities 抵触 → hit=false
