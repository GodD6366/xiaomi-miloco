...
def select_active_camera_dids(
    ...
    *,
    ...
    （``online_only`` 时）在线 + 镜头未关。``require_lan=True`` 看 ``online and lan_online``；
    ``False`` 只看云端 ``online``（放过 lan_online 陈旧的卡死态相机）。``awake_map``
    （did→镜头开关态）给出时，``awake`` 明确为 ``False`` 的相机被排除；``None``/``True``/
    未给出一律放行。``cap=True`` 时按 did 升序确定性截断到 ``MAX_ENABLED_CAMERAS``。
